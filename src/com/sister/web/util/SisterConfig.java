...
public class SisterConfig {
	...
	static {
		try {
			File file = new File(System.getProperty("catalina.home") + "/conf/SisterConfig.properties");
			if (file.exists()) {
				 ...
				 * use default config
				 */
				in = SisterConfig.class.getClassLoader().getResourceAsStream("SisterConfig.properties");
			}
			config.load(in);
